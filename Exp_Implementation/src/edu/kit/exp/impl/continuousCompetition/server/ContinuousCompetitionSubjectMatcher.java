 ...
 */
public class ContinuousCompetitionSubjectMatcher extends SubjectGroupMatcher {

    private static final Logger log4j = LogManager.getLogger(ContinuousCompetitionSubjectMatcher.class.getName());
     ...
     * @param roles
     */
    public ContinuousCompetitionSubjectMatcher(List<String> roles) {
        super(roles);
    }
    ...
    public List<SubjectGroup> setupSubjectGroups(Period period, List<Subject> subjects) throws Exception {
        ...
        // Check illegal state
        if (subjectsRole1.size() != subjectsRole2.size() || subjectsRole1.size() != subjectsRole3.size() || subjectsRole2.size() != subjectsRole3.size() ) {
